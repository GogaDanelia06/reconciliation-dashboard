...
-- ==================== ACCESS ====================
-- This is a single-tenant internal dashboard with no end-user auth (auth was
-- optional in the brief). We keep RLS disabled so the anon/publishable key can
-- read and reconcile. In a multi-tenant production setup this would instead be
-- RLS ON + policies scoped to the authenticated org.
