...
## სად დევს matching logic და რატომ (ქართულად)

მატჩინგი ხდება **მონაცემთა ბაზაში**, PostgreSQL ფუნქციის (`run_matching()`) სახით,
რომელსაც აპლიკაცია იძახებს `supabase.rpc("run_matching")`-ით.

**წესი:** ტრანზაქცია ემთხვევა კომპანიას, როცა `sender_inn = company.tax_id`.
მატჩინგი ხდება **მხოლოდ საიდენტიფიკაციო კოდით, არა სახელით** — რადგან ერთი და იგივე
კომპანია გვხვდება სხვადასხვა დასახელებით („შპს გეოტრანსი", „გეოტრანსი (ფილიალი)",
„გეოტრანსი"), მაგრამ ერთი ს/კ-ით.

**რატომ ბაზაში და არა კლიენტზე:**

1. **ეს არის set-operation.** ყველა ტრანზაქციის დამატჩვა ერთი
   `UPDATE … FROM companies WHERE sender_inn = tax_id`-ია. კლიენტზე გაკეთება ნიშნავდა
   ყველა row-ის წამოღებას, JS-ში ციკლს და ათობით ცალკეული UPDATE-ის უკან გაგზავნას —
   მეტი round-trip და partial-failure-ის რისკი.
2. **ატომურობა.** მთელი ოპერაცია ერთ statement-ში სრულდება — ან სრულად, ან საერთოდ.
3. **იდემპოტენტურობა.** ფუნქცია ეხება მხოლოდ `unmatched` row-ებს, ამიტომ თავიდან გაშვება
   არასდროს გადააწერს ხელით დამატჩულს (`manual`) ან `ignored`-ს.

აპლიკაციის მხარე თხელია: typed service (`lib/services/matching.ts`) იძახებს RPC-ს,
ხოლო TanStack Query-ის mutation წარმატების შემდეგ აბათილებს (invalidate) შესაბამის
query-ებს → UI თავისით ნახლდება.

იგივე მიდგომით, **„მოსალოდნელი vs ფაქტობრივის"** გათვლაც ბაზაშია —
`get_expected_vs_actual(p_month)` ფუნქციაში — რადგან „თვეში აქტიური ხელშეკრულების"
განსაზღვრა (`end_date`-ის გათვალისწინებით) თარიღებზე დამყარებული ლოგიკაა და ჯობია
მონაცემებთან ახლოს იდგეს.

## რა გავაკეთეთ (ქართულად)

**მონაცემთა ბაზა (Supabase / Postgres):** სამი ცხრილი (`companies`, `contracts`,
`bank_transactions`) — schema-ს, ინდექსებისა და seed-მონაცემების (15 კომპანია, 18
ხელშეკრულება, 89 ტრანზაქცია) ჩათვლით — და სამი DB ფუნქცია: `run_matching()`,
`reset_matching()` და `get_expected_vs_actual()`.

**ავტო-მატჩინგი (dry-run → review → save):** ღილაკზე „Run auto-matching" დაჭერისას
იხსნება დიალოგი, სადაც: (1) ირჩევ თვე(ებ)ს, (2) **Preview** აჩვენებს რომელი
ტრანზაქციები დაემატჩება — რაიმეს ჩაწერის გარეშე, (3) **Save** მხოლოდ მაშინ წერს
ბაზაში. **Reset** ღილაკი აბრუნებს ავტომატურად დამატჩულ ტრანზაქციებს `unmatched`-ში
(ხელით დამატჩული და `ignored` ტრანზაქციები არ ეხება).

**დეშბორდი:**
- სტატისტიკის ზოლი — სულ/დამთხვეული/შეუსაბამო რაოდენობა და თანხა, match rate
- **Reconciliation progress** — SVG დონატ-დიაგრამა match rate-ისთვის + სვეტოვანი
  სქემა სტატუსების მიხედვით
- ტრანზაქციების ცხრილი — sortable (თარიღი/თანხა), filterable (სტატუსი), searchable
  (გამგზავნი/ს.კ.), ფერადი სტატუს-ბეჯები, თითო row-ზე manual match/ignore/reset
- **Expected vs Actual** — კომპანიის მიხედვით, **Coverage**-ბარით (რამდენი
  გადაიხადა მოსალოდნელთან შედარებით — მწვანე/წითელი/ნაცრისფერი/ნარინჯისფერი
  კონტრაქტგარეშე გადახდისთვის), CSV export-ით
- **თვეების ნავიგაცია** — აპრილი/მაისი/ივნისი, ყველა სექცია არჩეულ თვეს ასახავს

**UI/UX:** sidebar-ზე დაფუძნებული layout (period + section ნავიგაცია scroll-spy-ით),
mobile-ზე drawer-ად იშლება და desktop-ზეც იმალება ერთი ღილაკით; **dark/light mode**
(system-aware, flash-ის გარეშე); responsive — ცხრილი ეტევა თავის ბლოკში დესკტოპზე
სქროლის გარეშე, ხოლო ვიწრო ეკრანზე მხოლოდ ცხრილი სქროლავს ჰორიზონტალურად.

**კოდის ორგანიზაცია:** components → hooks (TanStack Query) → services (typed
Supabase calls) → database (RPC). Zod ვალიდაცია ფილტრებისთვის, query-key factory
სწორი invalidation-ისთვის, optimistic updates rollback-ით.
...
- **Auto-matching** by tax ID (DB function) with a **dry-run → review → save** flow: pick which month(s) to match, preview the proposed matches (nothing is written yet), then commit. A **Reset** reverts auto matches (manual/ignored decisions are preserved), so reconciliation is fully reversible and replayable
