  ...
  return (
    <div className="min-h-screen">
        ...
        <header className="sticky top-0 z-30 border-b border-line bg-page/80 backdrop-blur">
          <div className="flex items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12a9 9 0 1 1-2.64-6.36M21 3v6h-6" /></svg>
                <span className="sm:hidden">Match</span>
                <span className="hidden sm:inline">Run auto-matching</span>
              </button>
            </div>
          </div>
        </header>
        ...
        <main className="mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
          ...
          {resetMatching.isError && (
            <p className="mb-4 rounded-lg bg-red-50 px-4 py-2 text-sm text-red-700 ring-1 ring-red-600/20 dark:bg-red-500/10 dark:text-red-400 dark:ring-red-500/20">
              {(resetMatching.error as Error).message}
            </p>
          )}
      ...
      <MatchDialog
        ...
        onSaved={(count) => setMatchFlash(`Matched ${count} new transaction(s).`)}
      />
    </div>
  );
}
